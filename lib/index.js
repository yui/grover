require('colors');

var good = "✔",
    bad = "✖",
    timer = function(start, end) {
        var total = end - start,
            diff = {}, str = '';
 
        diff.seconds_raw = (total/1000);

        diff.days = Math.floor(total/1000/60/60/24);
        total -= diff.days*1000*60*60*24;
 
        diff.hours = Math.floor(total/1000/60/60);
        total -= diff.hours*1000*60*60;
 
        diff.minutes = Math.floor(total/1000/60);
        total -= diff.minutes*1000*60;
 
        diff.seconds = Math.floor(total/1000);


        if (diff.hours) {
            str = diff.hours + ' hours, ' + diff.minutes + ' minutes, ' + diff.seconds + ' seconds';
        } else if (diff.minutes) {
            str = diff.minutes + ' minutes, ' + diff.seconds + ' seconds';
        } else {
            str = diff.seconds_raw + ' seconds';
        }

        return str;
    };

var printFail = function(name, message) {
    console.log('    ' + (name).red.bold);
    var m = message.split('\n');
    m.forEach(function(line) {
        console.log('       ' + line);
    });
};

var osenv = require('osenv');

var phantomVerson = '1.6.1';

module.exports = {
    fetch: function() {
        var platform = process.platform,
            file = '';
        console.log('phantomjs binary was not found, fetching for ' + platform + ' (' + process.arch + ')');
        switch (platform) {
            case 'linux':
                file = 'phantomjs-1.6.1-linux-' + ((process.arch === 'x64') ? 'x86_64' : 'i686') + '-dynamic.tar.bz2';
                break;
            case 'darwin';
                file = 'phantomjs-1.6.1-macosx-static.zip';
                break;
        }
    },
    canPrint: function(options, json) {
        json = json || {}
        var p = true;
        if (options.quiet) {
            p = false;
            if (json.failed) {
                p = true;
            }
        }
        if (options.silent) {
            p = false;
        }
        return p;
    },
    error: function(str) {
        console.error(str.red);
    },
    log: function(str) {
        console.log(str.bold.blue);
    },
    status: function(json, start, end) {
        var showCoverage;

        if (json.jscoverage) {
            showCoverage = true;
            var coverage = json.jscoverage;
            var cov = {
                lines: 0,
                hit: 0,
                miss: 0,
                percent: 0
            };
            for (var i in coverage) {
                cov.lines = cov.lines + coverage[i].lines;
                cov.miss = cov.miss + coverage[i].miss;
                cov.hit = cov.hit + coverage[i].hit;
            }
            cov.percent = Math.floor((cov.hit / cov.lines) * 100);
        }

        var str = (json.failed ? bad.bold.red : good.bold.green) + 
            ' ' + ('[' + json.name + ']:').bold[(json.failed ? 'red' : 'blue')] +
            ' Passed: ' + (json.passed ? (''+json.passed).green : json.passed) +
            ' Failed: ' + (json.failed ? (''+json.failed).red : json.failed) +
            ' Total: ' +  json.total +
            (' (ignored ' + json.ignored + ')').grey;

            if (showCoverage) {
                str += (' ' + cov.percent + '%').blue + ' ' + ((cov.hit + '').green + '/' + (cov.miss + '').red + '/' + cov.lines).green;
                //str += (' ' + cov.percent + '%').blue;
            }

        console.log(str);
        
        if (json.failed) {
            if (json.error) {
                printFail('Javascript Error', json.error);
            } else {
                Object.keys(json).forEach(function(key) {
                    if (typeof json[key] === 'object') {
                        for (var i in json[key]) {
                            if (json[key][i].failed) {
                                for (var t in json[key][i]) {
                                    if (json[key][i][t].result === 'fail') {
                                        var res = json[key][i][t];
                                        printFail(res.name, res.message);
                                    }
                                }
                            } else {
                                if (json[key][i].result === 'fail') {
                                    var res = json[key][i];
                                    printFail(res.name, res.message);
                                }
                            }
                        }
                    }
                });
            }
        }

        if (start && end) {
            module.exports.log('  [Timer] ' + timer(start, end));
        }
        
    }
}
