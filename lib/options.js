        ...
        while (args.length > 0) {
            ...
            switch (v) {
                case "--screwdriver":
                    ...
                    options.screwdriver = true;
                    options.outtype = 'JUnitXML';
                    if ('TEST_RESULTS_DIR' in process.env) {
                        options.outfile = path.resolve(process.env.TEST_RESULTS_DIR, 'junit.xml');
                    }
                    if ('COVERAGE_DIR' in process.env) {
                        options.coverdir = path.resolve(process.env.COVERAGE_DIR);
                    }
                    break;
                case "--no-color":
                    options.color = false;
