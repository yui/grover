...
var getIstanbulCoverageReport = function (json) {
    console.log('Generating Istanbul Coverage Report');
    process.exit(1);
    return report;
};
...
var getCoverageReport = function (json, prefix) {
    var out;
    if (isIstanbul(json.coverage)) {
        out = getIstanbulCoverageReport(json, prefix);
    } else {
        out = getYUITestLcovReport(getYUITestLcov(json.name, prefix, json.coverage));
    }
    return out;
};
